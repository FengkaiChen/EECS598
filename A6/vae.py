...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        x_flatten=x.flatten(1,3)
        x_=torch.cat((x_flatten,c),dim=1)
        encode=self.encoder(x_)
        mu=self.mu_layer(encode)
        logvar=self.logvar_layer(encode)
        z=reparametrize(mu,logvar)
        z_=torch.cat((z,c),dim=1)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    loss=(rec-0.5*torch.sum(1+logvar-mu**2-torch.exp(logvar)))/(x.shape[0])

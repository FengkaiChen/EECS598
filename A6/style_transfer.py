...
def content_loss(content_weight, content_current, content_original):
    ...
    loss=((content_current-content_original)**2).sum() * content_weight
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def gram_matrix(features, normalize=True):
    ...
    if normalize==True:
      gram=features.bmm(features.permute(0,2,1))/(H*W*C)
    else:
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, style in enumerate(style_layers):
      G=gram_matrix(feats[style])
      loss=(((G-style_targets[i])**2).sum())*style_weights[i]
      style_loss+=loss
    return style_loss
    ############################################################################
    #                               END OF YOUR CODE                           #
...
def tv_loss(img, tv_weight):
    ...
    loss=(torch.sum((img[:,:,1:,:]-img[:,:,:-1,:])**2) + torch.sum((img[:,:,:,1:]-img[:,:,:,:-1])**2))* tv_weight
    return loss

...
def compute_saliency_maps(X, y, model):
    ...
    N=X.shape[0]
    y_predict=model(X)
    loss=y_predict[range(N),y].sum()
    loss.backward()
    grad=torch.abs(X.grad.data)
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        ...
        y_predict=model(X_adv)
        label=y_predict.argmax(dim=1)
        if target_y==label:
            print("Iteration %d: target score %.3f, max score %.3f" %(i,y_predict[0,target_y],y_predict[0,label]))
            break
        loss=y_predict[0,target_y]
        loss.backward()
        dx=learning_rate*X_adv.grad / torch.norm(X_adv.grad,2)
        with torch.no_grad():
            X_adv+=dx
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    # Replace "pass" statement with your code
    y_predict=model(img)
    loss=y_predict[0,target_y]-l2_reg*((img*img).sum())
    loss.backward()
    with torch.no_grad():

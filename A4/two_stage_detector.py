...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        nn.init.normal_(Conv1.weight,mean=0.0, std=0.01)
        stem_rpn.append(Conv1)
        stem_rpn.append(nn.ReLU())
        for i in range(len(stem_channels)-1):
            Conv=nn.Conv2d(stem_channels[i],stem_channels[i+1],3,1,1)
        ...
        # Replace "pass" statement with your code
        Conv_obj=nn.Conv2d(stem_channels[-1],1*num_anchors,1)
        nn.init.normal_(Conv_obj.weight,mean=0.0, std=0.01)
        self.pred_obj=Conv_obj

        Conv_box=nn.Conv2d(stem_channels[-1],4*num_anchors,1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for keys in feats_per_fpn_level:
            input=feats_per_fpn_level[keys]
            B,C,H,W=input.shape
            rpn=self.stem_rpn(input)
            obj_out=self.pred_obj(rpn).flatten(1,3)
            box_out=self.pred_box(rpn).reshape((B,4,H*W*self.num_anchors)).swapaxes(1,2)
            
            object_logits[keys]=obj_out
            boxreg_deltas[keys]=box_out
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            new_h=area / new_w
            x1,x2,y1,y2=xc-new_w/2,xc+new_w/2,yc-new_h/2,yc+new_h/2
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    xx1 = torch.clip(boxes1[:,0:1]-boxes2[:,0:1].T,min=0)+boxes2[:,0:1].T
    yy1 = torch.clip(boxes1[:,1:2]-boxes2[:,1:2].T,min=0)+boxes2[:,1:2].T
    xx2 = torch.clip(boxes1[:,2:3]-boxes2[:,2:3].T,max=0)+boxes2[:,2:3].T
    yy2 = torch.clip(boxes1[:,3:4]-boxes2[:,3:4].T,max=0)+boxes2[:,3:4].T
    w = torch.clip(xx2-xx1,min=0)
    h = torch.clip(yy2-yy1,min=0)
    inter = w*h
    area1=area1.reshape(area1.shape[0],-1)
...
def rcnn_get_deltas_from_anchors(
    ...
    bw=bx2-bx1
    bh=(by2-by1)
    px1,py1,px2,py2=anchors[:,0],anchors[:,1],anchors[:,2],anchors[:,3]
    px=(px1+px2)/2
    py=(py1+py2)/2
    pw=px2-px1
    ph=(py2-py1)
    ...
    index=(gt_boxes==-1).nonzero()[:,0].unique()
    deltas[index]=torch.tensor([-1e8, -1e8, -1e8, -1e8],device=anchors.device)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        locations=get_fpn_location_coords(shape,strides_per_fpn_level,dtype=feats_per_fpn_level["p3"].dtype,device=feats_per_fpn_level["p3"].device)
        ...
        if self.training:
            ...
            index=torch.cat((fg_idx, bg_idx))

            gt_deltas=rcnn_get_deltas_from_anchors(anchor_boxes[index],matched_gt_boxes[index])
            gt_objectness=torch.zeros(gt_deltas.shape[0],device=pred_obj_logits.device)
            bg=(matched_gt_boxes[index][:,4]!=-1).nonzero()[:,0].unique()
            gt_objectness[bg]=1
            loss_obj = F.binary_cross_entropy_with_logits(pred_obj_logits[index].view(-1), gt_objectness, reduction="none")
            loss_box = F.l1_loss(pred_boxreg_deltas[index],gt_deltas, reduction="none")
    ...
    def predict_proposals(
        ...
        for _batch_idx in range(batch_size):
            ...
            for level_name in anchors_per_fpn_level.keys():
                ...
                W, H = image_size
                output_boxes[:,0] = output_boxes[:,0].clamp(min=0.)
                output_boxes[:,1] = output_boxes[:,1].clamp(min=0.)
                output_boxes[:,2] = output_boxes[:,2].clamp(max=W)
                output_boxes[:,3] = output_boxes[:,3].clamp(max=H)
                ...
                output_boxes = output_boxes[order]
                keep = torchvision.ops.nms(output_boxes,level_obj_logits,self.nms_thresh)


                proposals_per_fpn_level_per_image[level_name] = output_boxes[keep]
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        flatten_out=stem_channels[-1]*roi_size[0]*roi_size[1]
        linear=nn.Linear(flatten_out,num_classes+1)
    ...
    def forward(
        ...
        for level_name in feats_per_fpn_level.keys():
            ...
            roi_feats=torchvision.ops.roi_align(level_feats,level_props,self.roi_size,spatial_scale=1/level_stride)
            ##################################################################
            #                         END OF YOUR CODE                       #
            ##################################################################
        ...
        gt_class=(matched_gt_boxes[index,4]+1).to(dtype=int)

        loss_cls=F.cross_entropy(pred_cls_logits[index], gt_class)
    ...
    def inference(
        ...
        level_pred_scores=F.softmax(pred_cls_logits, dim=1)
        pred_scores,pred_classes=torch.max(level_pred_scores,dim=1)

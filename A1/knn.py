...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        dists[i,j]=(torch.sum((torch.flatten(x_train[i])-torch.flatten(x_test[j]))**2))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    flat_train=x_train.reshape(num_train,-1)
    flat_test=x_test.reshape(num_test,-1)
    for i in range(num_train):
      dists[i:]=torch.sum((flat_test-flat_train[i])**2,dim=1)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dists=torch.sum((flat_train**2),dim=1,keepdim=True)+torch.sum((flat_test**2),dim=1)-2*torch.matmul(flat_train,flat_test.T)
    #
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      knn=torch.topk(dists[:,i],k,dim=0,largest=False).indices
      close_y=y_train[knn]
      
      y_pred[i]=torch.mode(close_y)[0]
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for fold in range(num_folds):
        x_val_f=x_train_folds[fold]
        y_val_f=y_train_folds[fold]
        x_train_f=torch.cat(x_train_folds[:fold]+x_train_folds[fold+1:])
        y_trian_f=torch.cat(y_train_folds[:fold]+y_train_folds[fold+1:])
        
        classifier=KnnClassifier(x_train_f,y_trian_f)
        f_accuracy=classifier.check_accuracy(x_val_f,y_val_f,k=k)
        fold_accuracies.append(f_accuracy)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, value in (k_to_accuracies.items()):
        k_mean_accuracy.append(sum(value)/len(value))
    index=int(torch.argmax(torch.tensor(k_mean_accuracy)))
    
    best_k=list(k_to_accuracies.keys())[index]

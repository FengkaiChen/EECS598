...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idx0 = torch.tensor([1, 0, 3])  # Quick way to build [0, 1, 2, 3]
...
def make_one_hot(x: Tensor) -> Tensor:
    """
    ...
    # Replace "pass" statement with your code
    N=max(x)
    temp=torch.eye(N+1,dtype=torch.float32)
    y=temp[x,:]
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mu_j=x.sum(dim=0)/M
    temp=(x-mu_j)**2
    sigma_j=(temp.sum(dim=0)/(M-1))**0.5
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    Your implementation should not use any explicit Python loops (including
    comprehensions), and should not require more than O(N) memory. Hence, the
    use of Torch functions to create new tensors or expand existing tensors is
    not allowed.
    ...
    # Replace "pass" statement with your code
    x_new =0*x
    sorted, indices = torch.sort(x,stable=True)
    x_new[0] = 1
    x_new[1:] = torch.diff(sorted)             #differece > 0 means unique element, first element always unique
    uniques,indices = sorted[x_new > 0],indices[x_new > 0]    
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################

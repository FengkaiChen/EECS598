...
def svm_loss_vectorized(
    ...
    correct_class=scores[index,y][:,None]
    margins=scores-correct_class+1
    margins[margins<0]=0
    margins[index,y]=0
    loss=margins.sum()
    loss/=N
    ...
    margins[index,y]=-row_sum
    dW+=(X.T).matmul(margins)/N
    dW+=reg*2*W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def softmax_loss_vectorized(
    ...
    scores=torch.exp(scores)   
    index=torch.arange(num_train)
    correct_class=scores[index,y][:,None]
    total_scores=torch.sum(scores,dim=1)[:,None]
    softmax=correct_class/total_scores
    loss-=torch.sum(torch.log(softmax))

    temp=scores/total_scores
    temp[index,y] -= 1
    dW=X.T.matmul(temp)

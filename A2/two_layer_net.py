...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores-=torch.max(scores,dim=1)[0][:,None]
    scores=scores.exp()
    total_scores=torch.sum(scores,dim=1)[:,None]
    softmax = scores/total_scores
    index=torch.arange(N)
    loss=torch.sum(-torch.log(softmax[index,y]))
    ...
    softmax[index, y]-=1
    dsoftmax=softmax
    grads['b2']=torch.sum(dsoftmax,dim=0)/N
    grads['W2']=h1.T.matmul(dsoftmax)/N
    dh1=dsoftmax.matmul(W2.T)
    dh1[(h1<=0)]=0   
    grads['b1']=torch.sum(dh1,dim=0)/N
    grads['W1']=X.T.matmul(dh1)/N
    grads['W1']+=reg*2*W1
    grads['W2']+=reg*2*W2
...
def nn_get_search_params():
    ...
    regularization_strengths = [1e-2,1e-3]
...
def find_best_net(
    ...
    lr, hs, rs, ld = get_param_set_fn()
    for i in lr:
      for j in hs:
          for k in rs:
              for l in ld:
                  net = TwoLayerNet(3 * 32 * 32, j, 10, device=x_train.device,dtype=x_train.dtype)
                  stats=net.train(x_train, y_train, x_val,y_val,num_iters=3000, batch_size=1000, learning_rate=i, learning_rate_decay=l,reg=k, verbose=False)

                  subbest=max(stats['val_acc_history'])
                  if subbest > best_val_acc:
                    print(i,j,k,l)
                    best_net = net
                    best_stat = stats
                    best_val_acc = subbest
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################

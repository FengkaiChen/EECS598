...
def rnn_step_backward(dnext_h, cache):
    ...
    dtemp=(1-next_h.square())*dnext_h
    dx=dtemp.mm(Wx.T)
    dprev_h=dtemp.mm(Wh.T)
    dWx=(x.T).mm(dtemp)
    dWh=(prev_h.T).mm(dtemp)
    db=torch.sum(dtemp,dim=0)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    H=h0.shape[1]
    cache=[]
    h=torch.zeros((T,N,H),dtype=h0.dtype,device=h0.device)
    temp_h=h0
    for i in range(T):
      x_in=x[:,i,:].reshape(N,D)
      temp_h,cache_=rnn_step_forward(x_in,temp_h,Wx,Wh,b)
      cache.append(cache_)
      h[i]=temp_h
    h=h.swapaxes(0,1)
    ##########################################################################
...
def rnn_backward(dh, cache):
    ...
    dh_=torch.zeros((N,H),dtype=dh.dtype,device=dh.device)
    dWx, dWh, db=0,0,0
    for i in range(T):
      dh_temp=dh[:,-i-1,:]+dh_
      dx_t, dh_, dWx_, dWh_, db_=rnn_step_backward(dh_temp,cache[-i-1])
      dWx+=dWx_
      dWh+=dWh_ 
      db+=db_
      dx[-i-1]=dx_t
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss=F.cross_entropy(x.swapaxes(1,2),y,ignore_index=ignore_index,reduction='sum')/x.shape[0]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.image_encoder=ImageEncoder(image_encoder_pretrained,verbose=True)
        self.word_embedding=WordEmbedding(vocab_size, wordvec_dim)
        self.pool=nn.AvgPool2d(4)
        self.feat=nn.Linear(input_dim, hidden_dim)
        ...
        self.A = nn.Conv2d(input_dim, hidden_dim, (1, 1))
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == 'attn':
          ...
          pool = self.pool(feat)
          N,D=pool.shape[0],pool.shape[1]
          h0=self.feat(pool.reshape(N,D))

        word=self.word_embedding(captions_in)
    ...
    def sample(self, images, max_length=15):
        ...
        start= self._start*torch.ones(N,dtype=torch.long)
        word = self.word_embedding(start)
        ...
        if self.cell_type == "rnn":
            ...
            prev_h=h0
            H=h0.shape[1]
            prev_c= torch.zeros((N,H),dtype=images.dtype,device=images.device)
        elif self.cell_type == "attn":
            A=self.A(feats)
        ...
        for i in range(max_length):
            if self.cell_type == "rnn":
                next_h = self.rnn.step_forward(word, prev_h)
            elif self.cell_type == "lstm":
                next_h, next_c = self.lstm.step_forward(word, prev_h, prev_c)
                prev_c = next_c
            elif self.cell_type == "attn":
                attn,weight=dot_product_attention(prev_h,A)
                next_h, next_c = self.attn.step_forward(word, prev_h, prev_c,attn)
                attn_weights_all[:, i, :, :] = weight
                prev_c = next_c
               
            
            scores=self.affine(next_h)
            idx=torch.argmax(scores,dim=1)
            word = self.word_embedding(idx)
            captions[:, i] = idx
            prev_h=next_h
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        N,H=prev_h.shape
        temp=x.mm(self.Wx)+prev_h.mm(self.Wh)+self.b
        it,ft,ot,gt=torch.sigmoid(temp[:,0:H]),torch.sigmoid(temp[:,H:2*H]),torch.sigmoid(temp[:,2*H:3*H]),torch.tanh(temp[:,3*H:4*H])
        next_c=ft*prev_c+it*gt
        next_h=ot*torch.tanh(next_c)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        temp_h,temp_c=h0,c0
        for i in range(T):
            temp_h, temp_c=self.step_forward(x[:,i,:],temp_h,temp_c)
            hn[:,i,:]=temp_h
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def dot_product_attention(prev_h, A):
    ...
    attn_weights=torch.zeros((N,4,4),dtype=prev_h.dtype,device=prev_h.device)

    prev_h=prev_h.reshape((N,1,H))
    A=A.flatten(2,3)
    attn_weights=F.softmax((prev_h.bmm(A)).reshape(N,-1)/(H**0.5),dim=1).reshape(N,4,4)
    M_= F.softmax(prev_h.bmm(A).reshape(N,-1,1)/(H**0.5),dim=1)
    # print(M_.shape)
    # print(A.shape)
    attn=A.bmm(M_).reshape(N,H)
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        N,H=prev_h.shape
        temp=x.mm(self.Wx)+prev_h.mm(self.Wh)+attn.mm(self.Wattn)+self.b
        it,ft,ot,gt=torch.sigmoid(temp[:,0:H]),torch.sigmoid(temp[:,H:2*H]),torch.sigmoid(temp[:,2*H:3*H]),torch.tanh(temp[:,3*H:4*H])
        next_c=ft*prev_c+it*gt
        next_h=ot*torch.tanh(next_c)
        ######################################################################
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for i in range(T):
            attn,_=dot_product_attention(temp_h,A)
            temp_h, temp_c=self.step_forward(x[:,i,:],temp_h,temp_c,attn)
            hn[:,i,:]=temp_h
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################

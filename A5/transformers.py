...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    K,M=query.shape
    weight=torch.zeros(key.shape[0],dtype=query.dtype,device=query.device)
    out=torch.zeros((K,M),dtype=query.dtype,device=query.device)
    for i in range(K):
        for j in range(key.shape[0]):
            scaled=query[i].dot(key[j])/(M**0.5)
            weight[j]=scaled
        weight=torch.softmax(weight,dim=0)
        out[i]=weight.matmul(value)
...
def scaled_dot_product_two_loop_batch(
    ...
    # Replace "pass" statement with your code
    N,K,M=query.shape
    weight=torch.zeros((N,key.shape[1]),dtype=query.dtype,device=query.device)
    out=torch.zeros((N,K,M),dtype=query.dtype,device=query.device)
    for i in range(K):
        for j in range(key.shape[1]):
            scaled=(query[:,i].reshape(N,1,M)).bmm(key[:,j].reshape(N,M,1))/(M**0.5)
            weight[:,j]=scaled.reshape(N)
        weight=torch.softmax(weight,dim=1)
        
        out[:,i,:]=(weight.reshape(N,1,key.shape[1])).bmm(value).reshape(N,M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    # Replace "pass" statement with your code
    weights_softmax=torch.softmax(scaled,dim=2)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        nn.init.uniform_(self.q.weight, a = -c_q, b = c_q)
        self.k=nn.Linear(dim_in,dim_q)
        nn.init.uniform_(self.k.weight, a = -c_q, b = c_q)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        for i in range(num_heads):
            self.list.append(SelfAttention(dim_in,dim_out,dim_out))

        self.lin=nn.Linear(dim_out*num_heads,dim_in)
        c=(6/(dim_in+dim_out*num_heads))**0.5
        nn.init.uniform_(self.lin.weight, a = -c, b = c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        for single in (self.list):
            out.append((single(query,key,value,mask)))
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        std=((x-mean)**2).mean(dim=-1).sqrt().unsqueeze(-1) + self.epsilon
        y= (x-mean)/std*self.scale+self.shift
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        self.lin1=nn.Linear(inp_dim,hidden_dim_feedforward)
        self.relu=nn.ReLU()
        self.lin2=nn.Linear(hidden_dim_feedforward,inp_dim)
        nn.init.uniform_(self.lin1.weight, a = -c, b = c)
        nn.init.uniform_(self.lin2.weight, a = -c, b = c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.mult=MultiHeadAttention(num_heads,emb_dim,int(emb_dim/num_heads))
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        temp=self.mult(x,x,x)
        temp1=self.Layernorm1(temp+x)
        drop=self.dropout(temp1)
        temp2=self.feedforward(drop)
        temp3=self.Layernorm2(temp2+drop)
        y=self.dropout(temp3)
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        temp=self.attention_self(dec_inp,dec_inp,dec_inp,mask)
        temp1=self.norm1(temp+dec_inp)
        temp2=self.dropout(temp1)

        temp3=self.attention_cross(temp2,enc_inp,enc_inp)
        temp4=self.norm2(temp3+temp2)
        temp5=self.dropout(temp4)

        temp6=self.feed_forward(temp5)
        temp7=self.norm3(temp6+temp5)
        y=self.dropout(temp7)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        temp=self.encoder(q_emb_inp)
        mask = get_subsequent_mask(ans_b[:, :-1])
        dec_out = self.decoder(a_emb_inp, temp, mask)
        dec_out = dec_out.flatten(0, 1)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################

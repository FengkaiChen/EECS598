...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hid,cache_1=Linear.forward(X, w1, b1)
        relu_hid,cache_2=ReLU.forward(hid)
        scores,cache_3=Linear.forward(relu_hid, w2, b2)
        ...
        dx_r=ReLU.backward(dx,cache_2)
        _,grads["W1"],grads["b1"]=Linear.backward(dx_r,cache_1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        dim_t=[input_dim]
        dim_t+=hidden_dims
        dim_t.append(num_classes)
        for i in range(1,len(dim_t)):
          self.params['W'+str(i)]=torch.normal(0.,weight_scale,(dim_t[i-1],dim_t[i]),dtype=dtype, device=device)
          self.params['b'+str(i)]=torch.zeros(dim_t[i],dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers):
          ...
          hid,cache[i]=Linear_ReLU.forward(hid, w, b)
          if self.use_dropout:
            hid,cache_d[i]=Dropout.forward(hid,self.dropout_param)
        i+=1
        w=self.params['W'+str(i)]
        b=self.params['b'+str(i)]
        scores,cache[i]=Linear.forward(hid, w, b)
        ...
        dx,grads['W'+str(self.num_layers)],grads['b'+str(self.num_layers)]=Linear.backward(dout,cache[self.num_layers])
        grads['W'+str(self.num_layers)]+=2*self.reg*self.params['W'+str(self.num_layers)]
        for j in range(1, self.num_layers):
          index=self.num_layers-j
          if self.use_dropout:
            dx = Dropout.backward(dx, cache_d[index])
          dx,grads['W'+str(index)],grads['b'+str(index)]=Linear_ReLU.backward(dx,cache[index])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict,update_rule=sgd,optim_config={'learning_rate': 1,},lr_decay=0.95,num_epochs=20, batch_size=400,print_every=400,device=device)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def adam(w, dw, config=None):
    ...
    m_correction=config['m']/(1-config['beta1']**config['t'])
    v_correction=config['v']/(1-config['beta2']**config['t'])
    next_w =w - config['learning_rate'] * m_correction / (v_correction.sqrt() + config['epsilon'])
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
